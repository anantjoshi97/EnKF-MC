...
fig1, axes = plt.subplots(Nst,2)

for i in range(0,Nst):
	ax = axes[i,0]
	ax.plot(np.arange(ITER+1), m_mu[:,i],'k',label="$\mu_t$")
	ax.plot(np.arange(ITER+1), np.flip(m_lambda[:,i]),'--r',label="$\lambda_{T-t}$")
	ax.plot(np.arange(ITER+1), m_pNplot[:,i],'b',label="$\hat{\mu}_t$")
	ax = axes[i,1]
	ax.plot(np.arange(ITER+1), m_perror[:,i],'k',label="error1 pc")
